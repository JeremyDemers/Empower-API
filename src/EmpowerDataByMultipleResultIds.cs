...
{
    public static class EmpowerDataByMultipleResultIds
    {
        public static List<Dictionary<string, object>> FetchData(string oracleConnectionString, string resultIds, int year, DatabaseService databaseService)
        {
            ...
            // Split the resultIds string by commas and convert each to an integer
            var resultIdList = resultIds.Split(',').Select(int.Parse).ToList();

            foreach (var schema in schemas)
            {
                foreach (var resultId in resultIdList)
                {
                    combinedData.AddRange(FetchDataForMultipleResultIds(oracleConnectionString, resultId, schema));
                }
            }
        ...
        private static List<Dictionary<string, object>> FetchDataForMultipleResultIds(string connectionString, int resultId, string schema)
        {
